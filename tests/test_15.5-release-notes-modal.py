...
def _patched_get_db(monkeypatch):
    ...
    including commits made internally by db.py/queries.py helpers — is ever
...
class TestSeedFeaturesIncludesNewColumns:
    def test_seed_features_completes_without_error(self, _patched_get_db):
        seed_features()
        # Read via the same patched proxy — under the no-op-commit isolation
        # strategy, seed_features()'s inserts are never committed, so a
        # separate connection under READ COMMITTED would not see them.
        cur = _patched_get_db.cursor()
    ...
    def test_seeded_rows_have_reports_for_releases(self, _patched_get_db):
        ...
        result = queries_module.get_all_features()
        release_numbers = {
            f["number"] for f in result if f["parent_number"] is not None
        }
        for row in result:
            if row["number"] in release_numbers:
                assert (
                    row["test_report"] is not None
                ), f"Release '{row['number']}' should have test_report"
                assert (
                    row["review_report"] is not None
                ), f"Release '{row['number']}' should have review_report"
            else:
                assert (
                    row["test_report"] is None
                ), f"Parent '{row['number']}' must have test_report=None"
                assert (
                    row["review_report"] is None
                ), f"Parent '{row['number']}' must have review_report=None"
...
class TestClickableDotConditions:
    def test_tested_dot_clickable_when_report_exists_and_parent_set(
    ...
    def test_reviewed_dot_clickable_when_report_exists_and_parent_set(
        self, client, _patched_get_db
    ):
        _insert_feature_row(
            _patched_get_db,
    ...
    def test_only_reviewed_clickable_when_only_review_report(
        self, client, _patched_get_db
    ):
        _insert_feature_row(
            _patched_get_db,
...
class TestModalMarkup:
    ...
    def test_report_content_is_html_escaped(self, client, _patched_get_db):
        _insert_feature_row(
            ...
            parent_number="MM05",
            tested_at=SHIPPED_TS,
            test_report=SAMPLE_TEST_REPORT_WITH_HTML,
        )
        body = _body(client.get("/roadmap"))
        table_html = _extract_table(body)
