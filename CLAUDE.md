...
.claude/
  ...
  features/
    ...
    user-thoughts/      # Free-form user input: text files, screenshots, notes
                        # File naming: Thoughts_YYYY-MM-DD.txt or Thoughts_YYYY-MM-DD_HH-MM.txt
                        # Each file must start with a "Created: YYYY-MM-DD HH:MM EST" line
...
| GET    | `/roadmap`                | `roadmap`             | logged-in   |
| GET/POST | `/features`             | `features`            | logged-in   |
| GET    | `/platform`               | `platform`            | logged-in   |
...
## Child → Parent Timestamp Propagation
...
## User Thought → Pipeline Reminder

When a new or updated thought appears in `user-thoughts/`, always run the full pipeline:

1. `/capture-thoughts [subfolder-name]` — captures and assigns a number
2. `/plan-release <number>` — decomposes into releases
3. `/create-spec <number.release> <slug>` — writes spec, creates branch
4. `/implement-feature <number.release>` — implements the change
5. `/test-feature <spec-name>` — writes + runs tests
6. `/code-review-feature <spec-name>` — parallel security + quality review
7. `/ship-feature` — commit, PR, squash merge, cleanup

**Never implement directly from a user thought without going through this pipeline.**
